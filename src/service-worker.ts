...
self.addEventListener('push', (event: any) => {
  let data = { title: 'Mikayla', body: 'Nouveau message reçu', data: { url: '/' } };
  try {
    if (event.data) {
      data = event.data.json();
    }
  } catch (e) {
    data = { title: 'Mikayla', body: event.data ? event.data.text() : 'Nouveau message', data: { url: '/' } };
  }

  event.waitUntil(
    (self as any).registration.showNotification(data.title || 'Mikayla', {
